...
**Floor-disable (active floors).** A building's **active floor count** is `displayedFloorLevel − disabledFloors`, clamped to `[0, displayedFloorLevel]`. `disabledFloors` (0 / absent = every built floor active) counts how many of the BUILT floors, from the top, are switched off. Throughput, power, storage capacity, and §4.5 cluster contribution all scale by the ACTIVE floor count, not the built count: `activeFloors` / `activeFloorLevel` in `buildings.ts`. A building with **0 active floors** is non-operational — it is `participatesInCluster`-excluded (alongside invalid buildings), draws/produces no power, runs no recipe, and freezes `operatingMs` accrual (no wear / maintenance degradation), exactly as the legacy binary disable did. A **partially** floor-disabled building (≥ 1 active floor) is operational and still wears. Switching floors off/on is **free and instantly reversible** via the inspector steppers (−/+ step one floor; Off = all off; Max = all on); each step calls `setBuildingActiveFloors`, which also moves the building's storage-cap contribution and clamps any inventory above the lowered cap. Crossing into **0 active floors** drains routes owned by the building (`drainRoutesForBuilding`, one-way — re-enabling does NOT restore them), matching demolish; re-enabling from a partial state never drains. Steppers are hidden while the building is under construction. (This replaces the earlier binary `disabled` flag, which is gone; old saves fold `disabled === true` into `disabledFloors = floorLevel + 1` at the v23→v24 migration.)
