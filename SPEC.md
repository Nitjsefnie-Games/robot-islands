...
| §5.2 Heat adjacency | L | M:N floor-scaled aggregation (consumer pools all adjacent sources; free-first then cheapest-coal; finite capacity split proportionally), fuel-burn ∝ delivered heat, boolean-heat consumers zero-load N:1 (#114). |
...
**Boolean-heat consumers.** A consumer with no `heatDemandKW` (e.g. Brick Kiln, Limekiln, Cement Mill, Charcoal Kiln, Ceramic Kiln) imposes no thermal load: it needs exactly one adjacent source, runs at throttle 1, bills a full `coalPerCycle` to that source (free source ⇒ zero), and occupies **zero** thermal capacity — so it never competes with a kW-demand consumer for a furnace's output. Source choice follows the same free-first, then cheapest-coal (lowest-ID tie-break) priority.
