...
describe('schema v25 — tradeOffers (§9.8 server-authoritative trades)', () => {
  ...
  it('current serialize emits schema v33', () => {
    expect(serializeWorld(makeInitialWorld(0), new Map(), 0).v).toBe(SCHEMA_VERSION);
  });
});
...
describe('schema v27 → v28 — global CO₂ atmosphere + extraEllipses biome (§7.4 / §3.6)', () => {
  ...
  it('SCHEMA_VERSION is 33 and 28/29/30/31/32/33 are in SUPPORTED_LOAD_VERSIONS', () => {
    ...
    expect(SUPPORTED_LOAD_VERSIONS.has(31)).toBe(true);
    expect(SUPPORTED_LOAD_VERSIONS.has(32)).toBe(true);
    expect(SUPPORTED_LOAD_VERSIONS.has(33)).toBe(true);
  });
  ...
  it('current serialize round-trips identity through serialize/loadWorld with a biome-stamped lobe', () => {
    const world = makeInitialWorld(0);
    const home = world.islands.find((s) => s.id === 'home')!;
    home.extraEllipses = [
...
describe('schema v31 → v32 — cluster conduits (§4.5)', () => {
  it('migrateV31toV32 defaults missing conduitLinks to empty', () => {
    const base = serializeWorld(makeInitialWorld(0), new Map(), 0, 0);
    const mines = [
      { id: 'm-coal', defId: 'coal_mine', x: 0, y: 0 },
      { id: 'm-iron', defId: 'iron_mine', x: 4, y: 0 },
    ];
    const homeBase = base.world.islands.find((i) => i.id === 'home')!;
    const home = {
      ...homeBase,
      buildings: [...homeBase.buildings, ...mines],
    };
    const v31 = {
      ...base,
      v: 31,
      world: {
        ...base.world,
        islands: base.world.islands.map((i) => (i.id === 'home' ? home : i)),
        // Omit conduitLinks to simulate a genuine v31 save.
        conduitLinks: undefined,
      },
      islandStates: [{ id: 'home', state: { buildings: mines.map((b) => ({ ...b })) } }],
    } as unknown as SerializedSnapshotV31;

    const out = migrateV31toV32(v31);
    expect(out.v).toBe(32);
    expect(out.world.conduitLinks).toEqual([]);
  });

  it('round-trips conduitLinks through serialize → JSON → deserialize', () => {
    const world = makeInitialWorld(0);
    world.conduitLinks = [{ a: 'b1', b: 'b2' }];
    const snap = serializeWorld(world, new Map(), 0, 0);
    expect(snap.v).toBe(SCHEMA_VERSION);
    expect(snap.world.conduitLinks).toEqual([{ a: 'b1', b: 'b2' }]);
    const json = JSON.parse(JSON.stringify(snap)) as SaveSnapshot;
    const { world: loaded } = deserializeWorld(json, 0, 0);
    expect(loaded.conduitLinks).toEqual([{ a: 'b1', b: 'b2' }]);
  });
});
...
describe('v32 → v33 migration (§2.5 founderId backfill)', () => {
    ...
    const v32 = {
      ...
      world: {
        ...
        islands: [
          ...
          {
            id: 'art-5', name: 'art-5', biome: 'coast', cx: 100, cy: 0,
            majorRadius: 6, minorRadius: 6, populated: true, discovered: true,
            buildings: [], modifiers: [], artificial: true,
          },
        ],
      },
  ...
  it('v33 round-trips founderId identically through serialize/load', () => {
    const world = makeInitialWorld(0);
    world.islands.push({
      id: 'art-9-9', name: 'art-9-9', biome: 'plains', cx: 100, cy: 0,
      majorRadius: 6, minorRadius: 6, populated: true, discovered: true,
      buildings: [], modifiers: [], artificial: true, founderId: 'home',
    } as IslandSpec);
    world.islands.push({
      id: 'merged', name: 'merged', biome: 'plains', cx: -100, cy: 0,
      majorRadius: 10, minorRadius: 10, populated: true, discovered: true,
      buildings: [], modifiers: [],
      extraEllipses: [
        { originId: 'art-7', biome: 'coast', major: 8, minor: 8, rotation: 0, offsetX: 20, offsetY: 0, founderId: 'home' },
      ],
    } as IslandSpec);
    const snap = serializeWorld(world, new Map(), 0, 0);
    expect(snap.v).toBe(SCHEMA_VERSION);
    const json = JSON.parse(JSON.stringify(snap)) as SaveSnapshot;
    const { world: loaded } = deserializeWorld(json, 0, 0);
    expect(loaded.islands.find((i) => i.id === 'art-9-9')!.founderId).toBe('home');
    expect(loaded.islands.find((i) => i.id === 'merged')!.extraEllipses![0]!.founderId).toBe('home');
  });
});

describe('schema v28 → v29 — home base-layout radius (§3.7)', () => {
  function v28Snapshot(mutate: (w: ReturnType<typeof makeInitialWorld>) => void): SerializedSnapshotV28 {
    const world = makeInitialWorld(0);
    mutate(world);
    const snap = serializeWorld(world, new Map(), 0, 0);
    // Strip the field a genuine v28 save never had, and stamp v28.
    const islands = snap.world.islands.map((i) => {
      const { baseLayoutRadius: _drop, ...rest } = i as { baseLayoutRadius?: number };
      return rest;
    });
    return { ...snap, v: 28, world: { ...snap.world, islands } } as unknown as SerializedSnapshotV28;
  }

  it('migrateV28toV29 stamps baseLayoutRadius=16 on home only', () => {
    const v28 = v28Snapshot(() => {});
    // Precondition: the synthetic v28 has no baseLayoutRadius anywhere.
    expect(v28.world.islands.every((i) => (i as { baseLayoutRadius?: number }).baseLayoutRadius === undefined)).toBe(true);
    const out = migrateV28toV29(v28);
    expect(out.v).toBe(29);
    const home = out.world.islands.find((i) => i.id === 'home')!;
    expect(home.baseLayoutRadius).toBe(16);
    // Every non-home island stays free of the field.
    for (const isl of out.world.islands) {
      if (isl.id === 'home') continue;
      expect(isl.baseLayoutRadius).toBeUndefined();
    }
  });

  it('does NOT touch a merged island\'s constituents/terrain', () => {
    // home carries an absorbed desert lobe; the migration must leave the lobe
    // entry byte-identical (no baseLayoutRadius, biome/originId intact).
    const v28 = v28Snapshot((w) => {
      const home = w.islands.find((s) => s.id === 'home')!;
      home.extraEllipses = [
        { biome: 'desert', originId: 'gen-1--3', major: 12, minor: 12, rotation: 0, offsetX: 27, offsetY: -35 },
      ];
    });
    const lobeBefore = JSON.stringify(
      v28.world.islands.find((i) => i.id === 'home')!.extraEllipses,
    );
    const out = migrateV28toV29(v28);
    const home = out.world.islands.find((i) => i.id === 'home')!;
    expect(home.baseLayoutRadius).toBe(16);
    // The lobe constituent is unchanged — same biome, originId, geometry.
    expect(JSON.stringify(home.extraEllipses)).toBe(lobeBefore);
  });

  it('a v28 save loads through loadWorld and home gains baseLayoutRadius', () => {
    const v28 = v28Snapshot(() => {});
    const json = JSON.parse(JSON.stringify(v28)) as SaveSnapshot;
    const { world: loaded } = deserializeWorld(json, 0, 0);
    const home = loaded.islands.find((i) => i.id === 'home')!;
    expect(home.baseLayoutRadius).toBe(16);
  });

  it('v33 round-trips identity (baseLayoutRadius survives serialize/deserialize)', () => {
    const world = makeInitialWorld(0);
    const snap = serializeWorld(world, new Map(), 0, 0);
    expect(snap.v).toBe(SCHEMA_VERSION);
...
describe('schema v30 → v31 — split mine into iron_mine / coal_mine (§8.1)', () => {
  it('reclassifies each `mine` by footprint terrain (coal wins; updates spec + state)', () => {
    const base = serializeWorld(makeInitialWorld(0), new Map(), 0, 0);
    // Two legacy `mine` buildings. Force terrain via tileOverrides (precedence
    // over biome): m-coal footprint (0,0)-(1,1) has one coal tile → coal_mine
    // (coal wins); m-iron footprint (4,0)-(5,1) is all ore → iron_mine.
    const mines = [
      { id: 'm-coal', defId: 'mine', x: 0, y: 0 },
      { id: 'm-iron', defId: 'mine', x: 4, y: 0 },
    ];
    const homeBase = base.world.islands.find((i) => i.id === 'home')!;
    const home = {
      ...homeBase,
      buildings: [...homeBase.buildings, ...mines],
      tileOverrides: {
        '0,0': 'coal', '1,0': 'ore', '0,1': 'ore', '1,1': 'ore',
        '4,0': 'ore', '5,0': 'ore', '4,1': 'ore', '5,1': 'ore',
      },
    };
    const v30 = {
      ...base,
      ...
      // Mirror onto a per-island state entry to prove both copies reclassify.
      islandStates: [{ id: 'home', state: { buildings: mines.map((b) => ({ ...b })) } }],
    } as unknown as SerializedSnapshotV30;

    const out = migrateV30toV31(v30);
    expect(out.v).toBe(31);
    const outHome = out.world.islands.find((i) => i.id === 'home')!;
    expect(outHome.buildings.find((b) => b.id === 'm-coal')!.defId).toBe('coal_mine');
    expect(outHome.buildings.find((b) => b.id === 'm-iron')!.defId).toBe('iron_mine');
    // State-side copy reclassified identically.
    const outState = out.islandStates.find((e) => e.id === 'home')!.state;
    expect(outState.buildings.find((b) => b.id === 'm-coal')!.defId).toBe('coal_mine');
    expect(outState.buildings.find((b) => b.id === 'm-iron')!.defId).toBe('iron_mine');
  });

  it('SCHEMA_VERSION is 33 and 30/31/32/33 are in SUPPORTED_LOAD_VERSIONS', () => {
    expect(SCHEMA_VERSION).toBe(33);
    expect(SUPPORTED_LOAD_VERSIONS.has(30)).toBe(true);
    expect(SUPPORTED_LOAD_VERSIONS.has(31)).toBe(true);
    expect(SUPPORTED_LOAD_VERSIONS.has(32)).toBe(true);
...
describe('serialize → JSON → deserialize round-trip', () => {
  ...
  it('round-trips the §4.6 ignoreCapOverrides map; absent stays absent (defaults)', () => {
    ...
    homeSpec.buildings.push(
      { id: 'b-forced', defId: 'iron_mine', x: 0, y: 0, ignoreCapOverrides: { iron_ore: true } },
      { id: 'b-plain', defId: 'iron_mine', x: 1, y: 0 }, // no overrides → defaults apply
    );
    const snap = serializeWorld(world, new Map(), 0);
    const json = JSON.parse(JSON.stringify(snap)) as SaveSnapshot;
    const { world: restored } = deserializeWorld(json, 0, 0);
...
describe('migrateV13toV14', () => {
  it('resets per-island level + xp + skill-tree progression; preserves buildings/inventory', () => {
    ...
    playerLon: null,
      ...
      islandStates: [{
        ...
        state: {
          ...
          funnelPending: {},
          starterInventoryGrace: {},
          buildings: [{ id: 'mine-1', defId: 'iron_mine', x: 2, y: 2 }],
    ...
    expect(migrated.buildings[0]!.defId).toBe('iron_mine');
  });
});
...
describe('PlacedBuilding.floorLevel persistence', () => {
  it('floorLevel survives save→load', () => {
    const world = makeInitialWorld(0);
    const home = world.islands.find((s) => s.id === 'home')!;
    home.buildings.push({ id: 'mine-1', defId: 'iron_mine', x: 2, y: 2, floorLevel: 3 });
    const snap = serializeWorld(world, new Map(), 0, 0);
    const json = JSON.parse(JSON.stringify(snap)) as SaveSnapshot;
    const { world: restored } = deserializeWorld(json, 0, 0);
    const restoredHome = restored.islands.find((s) => s.id === 'home')!;
...
describe('v17 -> v18 migration', () => {
  ...
  it('round-trip: building with queued/queueSeq and nextQueueSeq on island state survive serialize→loadWorld', () => {
    ...
    homeSpec.buildings.push({ id: 'mine-q1', defId: 'iron_mine', x: 5, y: 5, queued: true, queueSeq: 1 });
    const homeState = makeInitialIslandState(homeSpec, 0);
  ...
  it('a building with NO queue fields loads as not-queued (absent ≡ default, no backfill)', () => {
    const world = makeInitialWorld(0);
    const homeSpec = world.islands.find((s) => s.id === 'home')!;
    homeSpec.buildings.push({ id: 'mine-noq', defId: 'iron_mine', x: 6, y: 6 });
    const snap = serializeWorld(world, new Map(), 0, 0);
    const json = JSON.parse(JSON.stringify(snap)) as SaveSnapshot;
    const { world: restored } = deserializeWorld(json, 0, 0);
    const rSpec = restored.islands.find((s) => s.id === 'home')!;
