...
export const TUTORIAL_STEPS: ReadonlyArray<TutorialStep> = [
  ...
  },
  {
    ...
    triggerCondition: (w) => hasBuilding(w, ['logger']) && !hasBuilding(w, ['iron_mine', 'coal_mine']),
    hint: 'Iron Mines pull iron ore (ore vein); Coal Mines pull coal (coal vein).',
    expectedAction: 'Place an Iron Mine on an ore vein or a Coal Mine on a coal vein (200 stone / 80 wood).',
    dismissalCondition: (w) => hasBuilding(w, ['iron_mine', 'coal_mine']),
    priority: 'critical',
    targetDefId: 'iron_mine',
  },
  {
    id: '08_tile_gate',
    mechanic: 'Tile-locked',
    triggerCondition: (w) => hasBuilding(w, ['iron_mine', 'coal_mine']),
  ...
  },
  {
    ...
    triggerCondition: (w) => hasBuilding(w, ['iron_mine', 'coal_mine']) && !hasBuilding(w, ['clay_pit_extractor']),
