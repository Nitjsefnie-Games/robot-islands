...
//
// Power check: an island is "powered" iff summed `def.power.produces` ≥ summed
// `def.power.consumes` across its placed buildings. Binary gate (no partial
// brownout), chosen because:
...
//   - §5.3 unified-pool / submarine-cable extensions live in `computeRates` /
//     `routes.ts`; the buoy tick stays at "is the island self-sufficient".
//     Cross-component / cable power can be wired through
//     `cableComponent.unified` later if buoy reveals matter in long brownouts.
//
// Antennas and Lighthouses skip the power check today (they emit regardless of
// brownout). The buoy is the first reveal-via-active-power building — the gate
// is intentional to match the spec.
//
...
/** Catalog id for the Sonar Buoy building def. Exported so catalog / UI /
 *  placement / picker code share one constant rather than the string literal. */
 ...
 *
 *  Notable simplifications vs `computeRates` (deliberate test-simplicity
 *  trade-offs; tighten if buoy reveal accuracy becomes load-bearing):
 ...
 *   - Construction-in-progress buildings (`constructionRemainingMs > 0`) are
 *     NOT excluded (§9.3); a half-built coal gen still "produces" here. */
