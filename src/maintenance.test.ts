...
function mkBuilding(
  defId: keyof typeof BUILDING_DEFS = 'iron_mine',
  operatingMs = 0,
): PlacedBuilding {
  return {
    id: 'b-test',
...
describe('maintenanceFactor', () => {
  it('returns 1.0 at operatingMs = 0', () => {
    const b = mkBuilding('iron_mine', 0);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBe(1.0);
  });

  it('returns 1.0 just below threshold (threshold - 1)', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD - 1);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBe(1.0);
  });

  it('returns 1.0 exactly at threshold (overshoot = 0)', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBe(1.0);
  });

  it('returns 0.5 at threshold + 4h (plateau)', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD + MAINTENANCE_DEGRADE_DURATION_MS);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBe(0.5);
  });

  it('returns 0.5 at threshold + 8h (well past plateau, clamped)', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD + 8 * HOUR);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBe(0.5);
  });

  it('linearly interpolates at threshold + 2h → 0.75', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD + 2 * HOUR);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBeCloseTo(0.75, 10);
  });

  it('linearly interpolates at threshold + 1h → 0.875', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD + 1 * HOUR);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBeCloseTo(0.875, 10);
  });

  it('linearly interpolates at threshold + 3h → 0.625', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD + 3 * HOUR);
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBeCloseTo(0.625, 10);
  });

  it('honours Eternal Servitor exemption regardless of operatingMs', () => {
    const b: PlacedBuilding = { ...mkBuilding('iron_mine', T1_THRESHOLD + 8 * HOUR), eternalServitor: true };
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBe(1.0);
  });
  ...
  it('uses each tier-specific threshold', () => {
    ...
    for (const tier of [1, 2, 3, 4, 5, 6] as ReadonlyArray<Tier>) {
      ...
      const fakeDef = { ...BUILDING_DEFS.iron_mine, tier };
      const under = mkBuilding('iron_mine', threshold - 1);
      const past = mkBuilding('iron_mine', threshold + MAINTENANCE_DEGRADE_DURATION_MS + 1);
  ...
  it('treats missing operatingMs (legacy save forward-compat) as 0', () => {
    const b: PlacedBuilding = { id: 'b', defId: 'iron_mine', x: 0, y: 0 };
    expect(maintenanceFactor(b, BUILDING_DEFS.iron_mine)).toBe(1.0);
  });
});
...
describe('accrueOperatingTime', () => {
  it('adds dt to operatingMs', () => {
    const b = mkBuilding('iron_mine', 1000);
  ...
  it('skips Eternal Servitor buildings', () => {
    const b: PlacedBuilding = { ...mkBuilding('iron_mine', 0), eternalServitor: true };
...
describe('tryAutoMaintain', () => {
  it('does nothing when under threshold', () => {
    ...
    inv.lubricant = 100;
    inv.bolt = 100;
    expect(tryAutoMaintain(b, BUILDING_DEFS.iron_mine, inv, 999)).toBe(false);
  ...
  it('does nothing past threshold when materials missing', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD + 10);
    const inv = blankInventory();
  ...
  it('consumes materials and resets operatingMs when bill is satisfied', () => {
    ...
    inv.lubricant = 10;
    inv.bolt = 10;
    expect(tryAutoMaintain(b, BUILDING_DEFS.iron_mine, inv, 12_345)).toBe(true);
  ...
  it('skips Eternal Servitor regardless of operatingMs', () => {
    const b: PlacedBuilding = { ...mkBuilding('iron_mine', T1_THRESHOLD * 10), eternalServitor: true };
    const inv = blankInventory();
    inv.lubricant = 100;
    inv.bolt = 100;
    expect(tryAutoMaintain(b, BUILDING_DEFS.iron_mine, inv, 999)).toBe(false);
...
describe('nextMaintenanceBoundaryMs', () => {
  it('returns the threshold when under it', () => {
    const b = mkBuilding('iron_mine', 1000);
    expect(nextMaintenanceBoundaryMs(b, BUILDING_DEFS.iron_mine)).toBe(T1_THRESHOLD);
  });
  ...
  it('returns the next ramp sub-boundary when in the ramp window', () => {
    ...
    const b = mkBuilding('iron_mine', T1_THRESHOLD + HOUR);
    expect(nextMaintenanceBoundaryMs(b, BUILDING_DEFS.iron_mine)).toBe(
  ...
  it('returns null past the plateau', () => {
    const b = mkBuilding('iron_mine', T1_THRESHOLD + 5 * HOUR);
    expect(nextMaintenanceBoundaryMs(b, BUILDING_DEFS.iron_mine)).toBe(null);
  });
...
describe('pickMostDegradedTarget', () => {
  ...
  it('returns null when no buildings are over threshold', () => {
    const b1 = mkBuilding('iron_mine', T1_THRESHOLD - 1);
  ...
  it('picks the single over-threshold building when only one exists', () => {
    const b1 = mkBuilding('iron_mine', T1_THRESHOLD - 1);            // factor 1.0
    const b2 = mkBuilding('iron_mine', T1_THRESHOLD + 1);            // ramp start
  ...
  it('picks the most-degraded (lowest factor) when multiple are over threshold', () => {
    ...
    const b1 = mkBuilding('iron_mine', T1_THRESHOLD + 1000);
    const b2 = mkBuilding('iron_mine', T1_THRESHOLD + MAINTENANCE_DEGRADE_DURATION_MS / 2);
    const b3 = mkBuilding('iron_mine', T1_THRESHOLD + MAINTENANCE_DEGRADE_DURATION_MS * 2);
    expect(pickMostDegradedTarget([b1, b2, b3], BUILDING_DEFS)).toBe(b3);
  });

  it('breaks ties by array order (first listed wins)', () => {
    const b1 = mkBuilding('iron_mine', T1_THRESHOLD + MAINTENANCE_DEGRADE_DURATION_MS * 2);
    const b2 = mkBuilding('iron_mine', T1_THRESHOLD + MAINTENANCE_DEGRADE_DURATION_MS * 2);
    expect(pickMostDegradedTarget([b1, b2], BUILDING_DEFS)).toBe(b1);
  });

  it('skips Eternal Servitors (their factor is always 1.0)', () => {
    const servitor = mkBuilding('iron_mine', T1_THRESHOLD * 100);
    (servitor as { eternalServitor?: true }).eternalServitor = true;
    const normal = mkBuilding('iron_mine', T1_THRESHOLD + 1);
  ...
  it('skips disabled buildings — a less-degraded enabled sibling is targeted (fix 4.4)', () => {
    ...
    const disabled = mkBuilding('iron_mine', T1_THRESHOLD + MAINTENANCE_DEGRADE_DURATION_MS * 2);
    (disabled as { disabledFloors?: number }).disabledFloors = displayedFloorLevel({ floorLevel: 0 });
    const enabled = mkBuilding('iron_mine', T1_THRESHOLD + 1000);
    expect(pickMostDegradedTarget([disabled, enabled], BUILDING_DEFS)).toBe(enabled);
  });

  it('skips invalid buildings (fix 4.4)', () => {
    const invalid = mkBuilding('iron_mine', T1_THRESHOLD + MAINTENANCE_DEGRADE_DURATION_MS * 2);
  ...
  it('returns null when every degraded building is disabled or invalid (fix 4.4)', () => {
    const disabled = mkBuilding('iron_mine', T1_THRESHOLD + 1000);
    (disabled as { disabledFloors?: number }).disabledFloors = displayedFloorLevel({ floorLevel: 0 });
    const invalid = mkBuilding('iron_mine', T1_THRESHOLD + 1000);
    (invalid as { invalid?: boolean }).invalid = true;
